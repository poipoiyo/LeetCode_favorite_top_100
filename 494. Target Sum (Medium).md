...
2. Solution 2 is based on 2D dynamic programming. Use two vector to memorize previous status and current status. Once a possibility j in previous vector, the current vector will be affected in j+nums[i] and j-nums[i]
3. Solution 3 moves the equation from 1+1+1+1-1 = 3 to 1+1+1+1 = 3+1. The problem becomes to 01 Knapsack problem (nums can do partition to match new target). New target is (sum(nums)+target)/2.
