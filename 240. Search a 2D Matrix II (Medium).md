...
3. We can start from the left-down or right-up side. For example, start from i = matrix\[0][n] (the right-up side). If target is larger than i, we go left. If smaller, we go down. Once it is out of the matrix range, return false.
