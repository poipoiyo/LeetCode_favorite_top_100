...
#### Ideas : 
 	...
 	1. digit : 
 		Since there will always be a "[" after digit, we have to memorize the number. If the number is larger than 10, the recording should be number = pre_num * 10 + (character-'0') 
 		
 	2. character : 
 		It's part of the answer, but it changed when it comes to []. Record it templately at the top of the stack which can be added immediately.
